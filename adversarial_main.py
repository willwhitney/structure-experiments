...
if opt.load is not None:
    i, adversary, autoencoder = load_adversarial_checkpoint(opt)
else:
    i = 0
...
while i < opt.max_steps:
    for sequence in train_loader:
        ...
        if last_autoencoder_output is None:
            last_autoencoder_output = autoencoder(sequence)
            continue


        # ---- train the autoencoder -----
        autoencoder_output = autoencoder(sequence)
        ...
        autoencoder_loss = recon_loss + 0.01 * adversarial_loss
