...
# Taken from https://github.com/gregversteeg/NPEET/blob/master/entropy_estimators.py
...
def cmi(x, y, z, k=3, base=2):
    ...
    tree = ss.cKDTree(points)
    dvec = [tree.query(point, k + 1, p=float('inf'))[0][k] for point in points]
    a, b, c, d = avgdigamma(zip2(x, z), dvec), avgdigamma(
        zip2(y, z), dvec), avgdigamma(z, dvec), digamma(k)
    return (-a - b + c + d) / log(base)
